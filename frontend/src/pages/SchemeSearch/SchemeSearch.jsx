...
const CATEGORY_LABELS = {
  education: 'Education', health: 'Health', women_empowerment: "Women's Empowerment",
  rural_development: 'Rural Development', skill_development: 'Skill Development',
  agriculture: 'Agriculture', environment: 'Environment', social_welfare: 'Social Welfare',
  housing: 'Housing', livelihood: 'Livelihood', children: 'Children', elderly: 'Elderly',
  disability: 'Disability', other: 'Other',
};

const LEVELS = [
  { value: '', label: 'All Levels' }, { value: 'central', label: 'Central Govt' },
  { value: 'state', label: 'State Govt' }, { value: 'district', label: 'District' },
];
      ...
      <div className="relative py-16 px-6 border-b border-slate-200 overflow-hidden bg-white">
        ...
        <div className="max-w-4xl mx-auto relative text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 border border-blue-100 text-blue-700 text-sm mb-5">
            <Globe className="w-4 h-4" /> Government Scheme Intelligence
          </div>
          <h1 className="text-4xl md:text-5xl font-extrabold font-display text-slate-900 mb-4">
            Find the Right <span className="text-blue-700">Government Scheme</span>
          </h1>
          <p className="text-slate-600 text-lg mb-8">Search 850+ central and state government schemes with AI-powered recommendations</p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex flex-wrap gap-2 mb-8">
          ...
          {categories.slice(0, 8).map((c) => (
            <button key={c._id} onClick={() => { setCategory(c._id); setPage(1); }}
              ...
              {CATEGORY_LABELS[c._id] || c._id} ({c.count})
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white border border-slate-200 rounded-2xl shadow-sm">
            ...
            <p className="text-slate-600 mb-6 font-medium">Try different keywords or clear your filters</p>
            <button onClick={clearFilters} className="btn-primary">Clear Filters</button>
          </div>
        ) : (
          <>
            <p className="text-slate-600 font-medium text-sm mb-4">{pagination?.total || 0} schemes found</p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {schemes.map((scheme) => (
        ...
        <div className="mt-12 bg-blue-50 border border-blue-200 rounded-2xl p-8 text-center shadow-sm">
          ...
          <h3 className="text-xl font-bold text-slate-900 mb-2">Get AI-Powered Recommendations</h3>
          <p className="text-slate-600 font-medium mb-5 text-sm">Tell our AI about your NGO's focus areas and get personalized scheme recommendations with eligibility analysis.</p>
          <Link to="/chat" className="btn-primary">
            <Sparkles className="w-4 h-4" /> Ask AI for Recommendations
          </Link>
        </div>
      </div>
    </div>
    ...
    <div className="bg-white rounded-2xl p-5 border border-slate-200 card-hover flex flex-col shadow-sm">
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <div className="flex flex-wrap gap-1 justify-end">
          <Badge variant={scheme.level === 'central' ? 'primary' : 'info'} className="text-xs capitalize">{scheme.level}</Badge>
          {scheme.deadline?.isOngoing && <Badge variant="success" className="text-xs">Ongoing</Badge>}
        </div>
      </div>
      ...
      <div className="space-y-1.5 mb-4">
        ...
        {scheme.benefits?.financialAmount > 0 && (
          <div className="flex items-center gap-2 text-xs text-emerald-700 font-semibold">
            ...
            <span>Up to ₹{scheme.benefits.financialAmount.toLocaleString('en-IN')}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-1 mb-4">
        {scheme.focusAreas?.slice(0, 3).map((f) => (
          <span key={f} className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 text-xs font-semibold capitalize">{f.replace('_', ' ')}</span>
        ))}
      </div>
