...
export default function Analytics() {
  ...
  if (loading) return <div className="flex items-center justify-center h-64"><LoadingSpinner size="lg" message="Loading analytics..." /></div>;
    ...
    <div className="space-y-6 fade-in-up">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">Real-time insights across all NGOs, schemes, and social impact</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard label="Active NGOs" value={s.activeNGOs?.toLocaleString('en-IN') || '—'} icon={Building2} color="indigo" change={8} />
        <StatsCard label="Total Projects" value={s.totalProjects?.toLocaleString('en-IN') || '—'} icon={BarChart3} color="emerald" change={15} />
        <StatsCard label="Active Schemes" value={s.activeSchemes?.toLocaleString('en-IN') || '—'} icon={Globe} color="amber" />
        <StatsCard label="Beneficiaries" value={s.totalBeneficiaries ? (s.totalBeneficiaries / 1000).toFixed(1) + 'K' : '—'} icon={Users} color="violet" change={22} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-slate-500 text-sm font-semibold">Total Funding Tracked</p>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-slate-500 text-sm font-semibold">Registered Users</p>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-slate-500 text-sm font-semibold">Avg. Impact Score</p>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <h2 className="text-lg font-bold text-slate-900 mb-4">Platform Growth — Last 7 Months</h2>
        <ResponsiveContainer width="100%" height={250}>
          <AreaChart data={monthlyTrend}>
            ...
            <Area type="monotone" dataKey="ngos" name="NGOs" stroke="#1d4ed8" fill="url(#ngoGrad)" strokeWidth={2} />
            <Area type="monotone" dataKey="projects" name="Projects" stroke="#10b981" fill="url(#projGrad)" strokeWidth={2} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <h3 className="text-base font-bold text-slate-900 mb-4">NGOs by State</h3>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={(data?.ngosByState || []).slice(0, 6)} layout="vertical">
              ...
              <Tooltip contentStyle={chartStyle} />
              <Bar dataKey="count" name="NGOs" fill="#1d4ed8" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <h3 className="text-base font-bold text-slate-900 mb-4">Projects by Status</h3>
          ...
          <div className="grid grid-cols-2 gap-1 mt-2">
            {(data?.projectsByStatus || []).map((s, i) => (
              <div key={i} className="flex items-center gap-1.5 text-xs">
                <div className="w-3 h-3 rounded-full shrink-0" style={{ background: COLORS[i] }} />
                <span className="text-slate-500 font-semibold capitalize">{s._id}</span>
                <span className="text-slate-900 font-bold ml-auto">{s.count}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <h3 className="text-base font-bold text-slate-900 mb-4">Schemes by Category</h3>
          <div className="space-y-3">
            {(data?.schemesByCategory || []).slice(0, 6).map((c, i) => {
              const total = (data?.schemesByCategory || []).reduce((s, x) => s + x.count, 0);
              const pct = Math.round((c.count / total) * 100);
              return (
                <div key={i}>
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="text-slate-500 font-semibold capitalize">{c._id?.replace('_', ' ')}</span>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <h2 className="text-lg font-bold text-slate-900 mb-4">Beneficiaries Reached — Monthly Trend</h2>
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={monthlyTrend}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
            <XAxis dataKey="month" stroke="#64748b" tick={{ fontSize: 12 }} />
            <YAxis stroke="#64748b" tick={{ fontSize: 12 }} />
            <Tooltip contentStyle={chartStyle} formatter={(v) => [v.toLocaleString('en-IN'), 'Beneficiaries']} />
